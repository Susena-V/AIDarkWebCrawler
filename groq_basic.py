...
def analyze_text(url, text):
    ...
    # Increase risk level if URL contains "http" or "https"
    if "http://" in url or "https://" in url:
        risk_level = "MEDIUM"  # Automatically increase risk level
    
    # Further refine risk level based on detected patterns
    if results["Emails"] or results["Credit Cards"]:
        risk_level = "HIGH"
    elif results["Phone Numbers"] or detected_keywords:
        risk_level = max(risk_level, "MEDIUM")  # Ensure risk doesn't decrease
